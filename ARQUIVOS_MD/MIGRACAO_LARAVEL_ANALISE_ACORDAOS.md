...
- [x] **0.4** Semear `acordao_analysis_system` em `ai_prompts` via `AiPromptsSeeder` (§14). _Gate:_ teste `AiPrompt::contentForKey(...)` não vazio + idempotência (2x não duplica). _(Texto em `AcordaoAnalyst::defaultInstructions()`; seeder referencia a classe.)_
...
### Fase 1 — Agente IA
- [x] **1.1** `AcordaoAnalyst` (`Agent` + `HasStructuredOutput`; `instructions()` + fallback; `schema()` erro + 5 seções; `provider()='openrouter'`; `model()` dinâmico). _Gate:_ teste com `AcordaoAnalyst::fake([...])` validando `$response->structured` (5 chaves + `erro`) e modelo resolvido. _(Prompt movido para `AcordaoAnalyst::defaultInstructions()`; seeder referencia a classe. Override de modelo via construtor ou `prompt(..., model: $slug)`.)_

### Fase 2 — QA
- [x] **2.1** `app/Support/SectionQa.php` (porte de `qa.py`). _Gate:_ testes unitários por regra (limites mín/máx por seção, cada frase proibida, frases padrão de modulação, frase de info ausente, menção a tribunal/tema no teaser, teaser→published). _(Métodos `validateTeaser`, `validateSection`, `teaserStatus`; 38 testes em `tests/Unit/SectionQaTest.php`.)_

### Fase 3 — Service (idempotência, anexos, custo)
- [x] **3.1** `AcordaoAnalysisService` (carrega tese+acórdãos, anexa todos os PDFs, hashes, resolve modelo, chama agente, trata `erro`, QA, custo rateado, grava seções). _Gate:_ testes com `AcordaoAnalyst::fake` + `Storage::fake('s3')`: gera 5 seções (teaser published, resto draft, `is_active=false`), grava tokens/custo/snapshots/`raw_usage`; idempotência pula seção existente (→ `done` sem custo); campo `erro` vira erro permanente; seção vazia é pulada. _(Retorna `AcordaoAnalysisResult`; exceção `AcordaoAnalysisPermanentException`; 7 testes em `AcordaoAnalysisServiceTest.php`.)_
