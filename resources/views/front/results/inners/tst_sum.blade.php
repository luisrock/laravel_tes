    ...
    <tr>
        <td>
            <h4 class="h5 mt-3 mb-2">
                <a href="{{ url('/sumula') }}/tst/{{ $sum['trib_sum_id'] }}"> {{ $sum['trib_sum_tipo'] }} nº
                    {{ $sum['trib_sum_numero'] }}</a>
            </h4>
            ...
            <span class="tes-clear tes-text-to-be-copied" style="display: none"
                data-spec="trim">{{ $sum['trib_sum_tipo'] }} nº {{ $sum['trib_sum_numero'] }}.
                {{ $sum['trib_sum_tema'] }}. TEXTO: {{ $sum['trib_sum_texto'] }}</span>
