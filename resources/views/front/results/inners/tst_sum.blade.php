    ...
    <tr>
        <td class="tw-block tw-bg-white tw-border tw-border-slate-200 tw-rounded-lg tw-p-6 hover:tw-border-brand-300 hover:tw-shadow-sm tw-transition-all">
            <h4 class="tw-text-lg tw-font-semibold tw-text-brand-700 tw-mb-3">
                <a href="{{ url('/sumula') }}/tst/{{ $sum['trib_sum_numero'] }}" class="hover:tw-text-brand-900 hover:tw-underline tw-transition-colors">
                    {{ $sum['trib_sum_tipo'] }} nº {{ $sum['trib_sum_numero'] }}
                </a>
            </h4>
            ...
            <div class="tw-flex tw-items-center tw-gap-2 tw-mt-4 tw-pt-4 tw-border-t tw-border-slate-100">
                ...
                <x-save-to-collection-btn type="sumula" tribunal="tst" :contentId="$sum['trib_sum_id'] ?? 0" />
            </div>

            <span class="tes-clear tes-text-to-be-copied tw-hidden" data-spec="trim">
                {{ $sum['trib_sum_tipo'] }} nº {{ $sum['trib_sum_numero'] }}. {{ $sum['trib_sum_tema'] }}. TEXTO: {{ $sum['trib_sum_texto'] }}
            </span>
        </td>
    </tr>
@endforeach
