    ...
    <tr>
        <td class="tw-block tw-bg-white tw-border tw-border-slate-200 tw-rounded-lg tw-p-6 hover:tw-border-brand-300 hover:tw-shadow-sm tw-transition-all">
            <h4 class="tw-text-lg tw-font-semibold tw-text-brand-700 tw-mb-3">
                <a href="{{ url('/sumula') }}/stf/{{ $sum['trib_sum_vinculante'] ? 'sv' : '' }}{{ $sum['trib_sum_numero'] }}" class="hover:tw-text-brand-900 hover:tw-underline tw-transition-colors">
                    {{ $sum['trib_sum_titulo'] }}
                </a>
            </h4>
            ...
            <div class="tw-flex tw-items-center tw-justify-between tw-flex-wrap tw-gap-2 tw-mt-4 tw-pt-4 tw-border-t tw-border-slate-100">
                <div class="tw-flex tw-items-center tw-gap-2">
                    ...
                    <x-save-to-collection-btn type="sumula" tribunal="stf" :contentId="$sum['trib_sum_id'] ?? 0" />
                </div>
            ...
            <span class="tes-clear tes-text-to-be-copied tw-hidden" data-spec="trim">{{ $sum['trib_sum_titulo'] }}. {{ $sum['trib_sum_texto'] }} Aprovada em {{ $sum['trib_sum_data'] }}</span>
        </td>
    </tr>
@endforeach
